...
from file_operations import save_list_to_file
...
def get_random_natural_number():
    return randint(0, integer_limit + 1)
...
def generate_test_data():
    # All possible integers to fit factorial in int64_t range <-2^64, 2^63-1>
    for i in range(integer_limit+1):
...
def data_generator():
    generate_test_data()
    save_list_to_file(test_data_list)
